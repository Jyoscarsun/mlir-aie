...
def linear(
    ...
    with mlir_mod_ctx() as ctx:
        ...
        def device_body():
            ...
            # Input X
            inX = object_fifo("inX", shim_tile, mem_tile, 2, x_ty)
            memX = object_fifo(
                "memX", 
                mem_tile, 
                compute_tile2, 
            ...
            # Input W
            inW = object_fifo("inW", shim_tile, mem_tile, 2, w_ty)
            memW = object_fifo(
                "memW", 
                mem_tile, 
                compute_tile2, 
            ...
            @core(compute_tile2, f"linear_{t}x{d}x{h}.o", stack_size=0xD00)
            def core_body():
                for _ in range_(0xFFFFFFFF):
                    for _ in range_(tiles) if tiles > 1 else range(1):
                        ...
                        for _ in (
                            ...
                            matmul(elem_inX, elem_inW, Y_intermediate)
                            memX.release(ObjectFifoPort.Consume, 1)
                            memW.release(ObjectFifoPort.Consume, 1)
                    
                    elem_inb = memB.acquire(ObjectFifoPort.Consume, 1)
                    elem_out = outY.acquire(ObjectFifoPort.Produce, 1)
                    row_wise_add_func(Y_intermediate, elem_inb, elem_out)
                    ...
                    #     elem_inb = memB.acquire(ObjectFifoPort.Consume, 1)
                    #     for _ in range_(T // t):
                    #         elem_inY = memY_interim.acquire(ObjectFifoPort.Consume, 1)
                    #         elem_outY = outY.acquire(ObjectFifoPort.Produce, 1)

                    #         row_wise_add_func(elem_inY, elem_inb, elem_outY)

                    #         outY.release(ObjectFifoPort.Produce, 1)
                    #         memY_interim.release(ObjectFifoPort.Consume, 1)
                    #     memB.release(ObjectFifoPort.Consume, 1)
